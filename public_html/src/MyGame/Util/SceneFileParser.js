...
SceneFileParser.prototype.parseCameraXML = function () {
...
SceneFileParser.prototype.parseSquaresXML = function (sqSet) {
...
SceneFileParser.prototype._getObjJson = function (el) {
    var theElm = this.mSceneXml[el];
    if (theElm.length === 0) {
        console.error("Warning: Level element:[" + tagElm + "]: is not found!");
    }
    return theElm;
...
SceneFileParser.prototype.parseCameraJson = function () {
    var camElm = this._getObjJson("Camera");
    ...
    // setup the camera from the information extracted from json
    var cam = new Camera(
        vec2.fromValues(cx, cy),  // position of the camera
        w,                        // width of camera
        viewport                  // viewport (orgX, orgY, width, height)
...
SceneFileParser.prototype.parseSquaresJson = function (sqSet) {
    var elm = this._getObjJson("Square");
    var i, j, x, y, w, h, r, c, sq;
    for (i = 0; i < elm.length; i++) {
        ...
        // setup the renderable and put it into the array
        sq.setColor(c);
        sq.getXform().setPosition(x, y);
        sq.getXform().setRotationInDegree(r); // In Degree
        sq.getXform().setSize(w, h);
